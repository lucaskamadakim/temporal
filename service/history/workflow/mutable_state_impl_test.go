...
func (s *mutableStateSuite) TestContinueAsNewMinBackoffExecutionCompletesBeforeExecutionTime() {
	...
	// StartTime later than now (e.g. clock skew) should not produce a negative lifetime.
	...
	// A delayed-start run that actually executed and closed before its ExecutionTime
	// should use now - StartTime as its lifetime, not a negative value.
